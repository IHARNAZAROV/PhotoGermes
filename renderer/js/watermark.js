...
function updateWmOverlay() {
    ...
    if (isText) {
        // Text mode — clear any image-mode styles that may linger
        labelEl.style.backgroundImage    = '';
        labelEl.style.backgroundSize     = '';
        labelEl.style.backgroundRepeat   = '';
        labelEl.style.backgroundPosition = '';
        labelEl.style.width              = '';
        labelEl.style.height             = '';
        labelEl.style.opacity            = '';
        labelEl.style.border             = '';
        // Restore default dashed border for text label
        ...
        const angle     = document.getElementById('wm-text-angle')?.value || '0';
        const tile      = document.getElementById('wm-tile')?.checked;
        const diagonal  = document.getElementById('wm-diagonal')?.checked;

        if (labelText) {
            labelText.textContent       = text || '© Ватермарк';
            labelText.style.fontFamily  = `'${family}', sans-serif`;
            labelText.style.fontSize    = size + 'px';
            labelText.style.fontWeight  = bold   ? '700' : '500';
            labelText.style.fontStyle   = italic ? 'italic' : 'normal';
            labelText.style.color       = color;
            labelText.style.opacity     = opacity;
            labelText.style.display     = '';
        }

        const effectiveAngle = diagonal ? 45 : +angle;
        labelEl.style.transform = `rotate(${effectiveAngle}deg)`;
        labelEl.style.display   = '';

        // Position
        const grid  = document.getElementById('wm-text-position-grid');
        const active = grid?.querySelector('.wm-pos-cell.active');
        applyPositionToOverlay(overlay, active?.dataset.pos || 'cc', tile);

    } else {
        // Image mode — show thumbnail in overlay if available
        const thumb   = document.getElementById('wm-upload-thumb');
        const hasSrc  = thumb?.src && !thumb.src.endsWith('#');
        ...
        const wVal    = document.getElementById('wm-img-width')?.value || '';
        const hVal    = document.getElementById('wm-img-height')?.value || '';
        const tile    = document.getElementById('wm-img-tile')?.checked;

        if (hasSrc) {
            labelText.textContent = '';
            labelEl.style.backgroundImage  = `url('${thumb.src}')`;
            labelEl.style.backgroundSize   = 'contain';
            labelEl.style.backgroundRepeat = 'no-repeat';
            labelEl.style.backgroundPosition = 'center';
            labelEl.style.width  = (wVal ? Math.min(wVal, 160) + 'px' : '120px');
            labelEl.style.height = (hVal ? Math.min(hVal, 120) + 'px' : '90px');
            ...
            labelEl.style.opacity = '';
            labelEl.style.border  = '';
            if (labelText) labelText.textContent = '[ Загрузите изображение ]';
        }
        labelEl.style.transform = `rotate(${angle}deg)`;

        const grid   = document.getElementById('wm-img-position-grid');
        const active = grid?.querySelector('.wm-pos-cell.active');
        applyPositionToOverlay(overlay, active?.dataset.pos || 'br', tile);
    }
}

// Map 9-grid pos key → flex alignment on the overlay container
const POS_MAP = {
...
function initWmDrag() {
    ...
    label.addEventListener('mousedown', e => {
        ...
        label.style.transform = label.style.transform; // keep rotation
    });
